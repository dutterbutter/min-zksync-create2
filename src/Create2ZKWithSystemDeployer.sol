...
contract Create2ZK {
    ...
    function deploy(bytes32 salt, bytes32 bytecodeHash, bytes calldata inputData)
        external
        payable
    ...
    function computeCreate2Address(address sender, bytes32 salt, bytes32 bytecodeHash, bytes32 constructorInputHash)
        external
        pure
        returns (address)
    {
        return L2ContractHelper.computeCreate2Address(sender, salt, bytecodeHash, constructorInputHash);
    }
}
